...
import { IUser, Usuarios } from "@libs/User";

const router = Router();
const usuarioInstance = new Usuarios();

router.get('/',async (_req, res) => {
    try {
        res.json(await usuarioInstance.getAllUsuarios());
    } catch (ex) {
        console.error(ex);
...
router.get('/byindex/:index', async (req, res) => {
  ...
  router.post('/new', async (req, res) => {
    try {
      const newUsuario = req.body as unknown as IUser;
  ...
  router.put('/update/:index', async (req, res) => {
    try {
      const { index } = req.params;
      const usuarioFromForm = req.body as IUser;
